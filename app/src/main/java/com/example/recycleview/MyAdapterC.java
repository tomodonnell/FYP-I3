...
public class MyAdapterC extends RecyclerView.Adapter<MyAdapterC.ExampleViewHolder> {

    //Creating variable to contain list, allowing us to pass to adapter
    private ArrayList<Clare_Item> mClare_Item;
    ...
    public MyAdapterC(ArrayList<Clare_Item> clareitem) {
        mClare_Item = clareitem;
    }

    @NonNull
    @Override
    //Passing layout from xml file
    public MyAdapterC.ExampleViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.attraction_profile, parent, false);
        MyAdapterC.ExampleViewHolder evh = new ExampleViewHolder(v);
        return evh;
    }

    @Override
    public void onBindViewHolder(@NonNull com.example.recycleview.MyAdapterC.ExampleViewHolder holder, int position) {

        //Passing values to our variables by setting equal to items from Province_Item.java
        Clare_Item currentItem = mClare_Item.get(position);
    ...
    public int getItemCount() {
        return mClare_Item.size();
    }

    //Creating filter list method
    public void filterList(ArrayList<Clare_Item> filteredList) {
        mClare_Item = filteredList;
