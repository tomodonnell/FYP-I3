...
public class MyAdapterL extends RecyclerView.Adapter<MyAdapterL.ExampleViewHolder> {

    //Creating variable to contain list, allowing us to pass to adapter
    private ArrayList<Limerick_Item> mLimerick_Item;
    ...
    public MyAdapterL(ArrayList<Limerick_Item> limerickitem) {
        mLimerick_Item = limerickitem;
    }

    @NonNull
    @Override
    //Passing layout from xml file
    public MyAdapterL.ExampleViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.attraction_profile, parent, false);
        MyAdapterL.ExampleViewHolder evh = new ExampleViewHolder(v);
        return evh;
    }

    @Override
    public void onBindViewHolder(@NonNull com.example.recycleview.MyAdapterL.ExampleViewHolder holder, int position) {

        //Passing values to our variables by setting equal to items from Province_Item.java
        Limerick_Item currentItem = mLimerick_Item.get(position);
    ...
    public int getItemCount() {
        return mLimerick_Item.size();
    }

    //Creating filter list method
    public void filterList(ArrayList<Limerick_Item> filteredList) {
        mLimerick_Item = filteredList;
